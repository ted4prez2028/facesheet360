...
export const useOrganizationalCommunication = () => {
  ...
  const fetchOrganizationalUsers = useCallback(async () => {
    if (!user?.id) return;
    ...
    try {
      ...
      if (orgError) {
        ...
        console.error('Error fetching organizational users:', orgError);
        return;
      }
      ...
      console.error('Error in fetchOrganizationalUsers:', err);
    } finally {
      setLoading(false);
    }
  }, [user?.id]);

  const fetchConversations = async (orgUsers: OrganizationalUser[]) => {
    if (!user?.id) return;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('organizational-updates')
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        (payload) => {
          if (payload.eventType === 'UPDATE') {
            const updatedUser = payload.new as OrganizationalUser;
  ...
  return {
    ...
    refreshData: fetchOrganizationalUsers
  };
};