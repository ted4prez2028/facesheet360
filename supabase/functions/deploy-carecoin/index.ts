...
// Pre-compiled CareCoin contract bytecode (ERC-20 with mint function)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (balance === 0n) {
      throw new Error('Insufficient funds for deployment. Please fund the deployer wallet with MATIC.');
    }
    ...
    console.log('Minting 100 CARE tokens to deployer:', deployerAddress);
    const mintAmount = ethers.parseUnits('100', decimals);
    const mintTx = await contract.mint(deployerAddress, mintAmount);
    await mintTx.wait();
    console.log('Mint completed:', mintTx.hash);
