...
const fetchAnalyticsData = async (userId: string, timeframe: string): Promise<AnalyticsData> => {
  try {
    ...
    const { data: patientTrendsData, error: patientTrendsError } = await supabase.rpc(
      'get_patient_trends', 
    ...
    const { data: appointmentData, error: appointmentError } = await supabase.rpc(
      ...
      { timeframe_param: timeframe, provider_id_param: userId }
    );
    
    if (appointmentError) throw appointmentError;

    // Get patient demographics - age distribution
    const { data: patientDemographicsData, error: demographicsError } = await supabase.rpc(
    ...
    const { data: diagnosisData, error: diagnosisError } = await supabase.rpc(
    ...
    const { data: providerPerformanceData, error: performanceError } = await supabase.rpc(
    ...
    // Get CareCoins data
    const { data: careCoinsData, error: careCoinsError } = await supabase.rpc(
      'get_care_coins_analytics',
      { user_id_param: userId, timeframe_param: timeframe }
    );
    
    if (careCoinsError) throw careCoinsError;

    // Get overview statistics
    const { data: overviewStats, error: overviewError } = await supabase.rpc(
      'get_analytics_overview',
      { provider_id_param: userId, timeframe_param: timeframe }
    );
    
    if (overviewError) throw overviewError;

    // If any of the RPC functions don't return data, use empty arrays or default values
    return {
